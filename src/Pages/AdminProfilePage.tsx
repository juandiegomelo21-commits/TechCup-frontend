...
const AdminProfilePage = () => {
  ...
  const [stats, setStats] = useState<Stats>({ users: 0, teams: 0, matches: 0, tournaments: 0 });
  ...
  useEffect(() => {
    ...
    ]).then(([usersRes, teamsRes, matchesRes, tournamentsRes]) => {
      const users = usersRes.status === 'fulfilled' ? (usersRes.value.data as unknown[]).length : 0;
      const teams = teamsRes.status === 'fulfilled' ? (teamsRes.value.data as unknown[]).length : 0;
      const matches = matchesRes.status === 'fulfilled' ? (matchesRes.value.data as unknown[]).length : 0;
      const tournaments = tournamentsRes.status === 'fulfilled' ? (tournamentsRes.value.data as unknown[]).length : 0;

      if (usersRes.status === 'fulfilled' && userId) {
        const players = usersRes.value.data as { id: string; fullname: string }[];
        const me = players.find(p => p.id === userId);
        if (me) setFullname(me.fullname);
      }

      setStats({ users, teams, matches, tournaments });
    });
  }, [userId]);

  const displayName = fullname || email.split('@')[0] || 'Organizador';
          ...
          <div style={{ padding: '16px 18px' }}>
            ...
            <div style={{ display: 'flex', gap: '12px', marginBottom: '10px' }}>
              <div style={{ flex: 1 }}>
                ...
                <p style={{ margin: 0, fontSize: '12px', color: '#333', fontFamily: "'Inter', sans-serif" }}>{displayName}</p>
              </div>
              <div style={{ flex: 1 }}>
