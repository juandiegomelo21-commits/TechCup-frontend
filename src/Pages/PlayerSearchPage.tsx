  ...
  return (
    <div style={{ width: '100vw', height: '100vh', position: 'relative', overflow: 'hidden', display: 'flex' }}>
      ...
      <img
        src={campoFutbol}
        alt="Jugador"
        style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', objectFit: 'cover', objectPosition: 'center', opacity: 0.22, zIndex: 1 }}
      />
      ...
      <div style={{ position: 'relative', zIndex: 10, width: '170px', minWidth: '170px', height: '100%', display: 'flex', flexDirection: 'column', padding: '20px 0' }}>
        ...
        <nav style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '2px', padding: '0 10px' }}>
          {[
            ...
            { label: 'Pagos', icon: '💳' },
            { label: 'Mercado', icon: '🛒', active: true },
        ...
        <div style={{ backgroundColor: 'rgba(255,255,255,0.92)', borderRadius: '14px', flex: 1, display: 'flex', flexDirection: 'column', padding: '16px', overflow: 'hidden', boxShadow: '0 10px 40px rgba(0,0,0,0.25)' }}>
          ...
          <div style={{ display: 'flex', gap: '10px', marginBottom: '14px', alignItems: 'flex-start' }}>
            ...
            <div style={{ position: 'relative', flex: 1, maxWidth: '340px' }}>
              ...
              />
              <div style={{ position: 'absolute', right: '0', top: '0', bottom: '0', width: '38px', backgroundColor: '#2ecc71', borderRadius: '0 8px 8px 0', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>
            ...
            <div style={{ position: 'relative' }}>
              <button
                ...
                style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '9px 14px', borderRadius: '8px', border: '1px solid #ddd', backgroundColor: '#f9f9f9', fontFamily: "'Inter', sans-serif", fontSize: '12px', cursor: 'pointer', color: '#333', fontWeight: 500, minWidth: '130px', justifyContent: 'space-between' }}
              >
                <span style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
                  ...
                  Filtros {activeFiltersCount > 0 && (
                    <span style={{ backgroundColor: '#2ecc71', color: '#fff', borderRadius: '50%', width: '16px', height: '16px', fontSize: '9px', display: 'inline-flex', alignItems: 'center', justifyContent: 'center' }}>
                      {activeFiltersCount}
                    </span>
                  )}
                </span>
                ...
                <div style={{ position: 'absolute', top: 'calc(100% + 6px)', left: 0, backgroundColor: '#fff', borderRadius: '10px', border: '1px solid #e0e0e0', boxShadow: '0 8px 24px rgba(0,0,0,0.12)', zIndex: 100, width: '220px', overflow: 'hidden' }}>
                  ...
                  <div>
                    ...
                    {openSection === 'Edad' && (
                      <div style={{ padding: '6px 14px 10px', borderTop: '1px solid #f0f0f0' }}>
                        <input type="number" placeholder="Ingrese un Número" value={filters.edad} onChange={(e) => setFilters((p) => ({ ...p, edad: e.target.value }))} min={15} max={40} style={{ width: '100%', padding: '7px 10px', borderRadius: '6px', border: '1px solid #ddd', fontSize: '12px', fontFamily: "'Inter', sans-serif", outline: 'none', boxSizing: 'border-box' }} />
                      </div>
                    )}
                  </div>
                  ...
                  <div>
                    ...
                    {openSection === 'Semestre' && (
                      <div style={{ padding: '6px 14px 10px', borderTop: '1px solid #f0f0f0' }}>
                        <input type="number" placeholder="Ingrese un Número" value={filters.semestre} onChange={(e) => setFilters((p) => ({ ...p, semestre: e.target.value }))} min={1} max={10} style={{ width: '100%', padding: '7px 10px', borderRadius: '6px', border: '1px solid #ddd', fontSize: '12px', fontFamily: "'Inter', sans-serif", outline: 'none', boxSizing: 'border-box' }} />
                      </div>
                    )}
                  </div>
                  <div style={{ height: '1px', backgroundColor: '#f0f0f0' }} />
                  ...
                  <div>
                    ...
                    {openSection === 'Posicion' && (
                      <div style={{ padding: '6px 14px 10px', borderTop: '1px solid #f0f0f0', display: 'flex', flexDirection: 'column', gap: '4px' }}>
                        {posiciones.map((pos) => (
                          <button key={pos} onClick={() => setFilter('posicion', pos)} style={{ padding: '5px 10px', borderRadius: '6px', border: 'none', cursor: 'pointer', fontFamily: "'Inter', sans-serif", fontSize: '11px', textAlign: 'left', backgroundColor: filters.posicion === pos ? '#2ecc71' : '#f5f5f5', color: filters.posicion === pos ? '#fff' : '#333' }}>
                            {pos}
                          </button>
                        ))}
                      </div>
            ...
            {filtered.length === 0 ? (
              ...
              filtered.map((player) => (
                <div key={player.id} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px 12px', borderRadius: '8px', backgroundColor: 'rgba(255,255,255,0.7)', border: '1px solid rgba(0,0,0,0.06)' }}>
                  <div style={{ width: '38px', height: '38px', borderRadius: '50%', backgroundColor: '#ccc', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '18px' }}>
                    👤
                  </div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    {player.username && (
                      <p style={{ margin: '0 0 2px 0', fontFamily: "'Inter', sans-serif", fontSize: '10px', color: '#888' }}>{player.username}</p>
                    )}
                    <p style={{ margin: 0, fontFamily: "'Inter', sans-serif", fontSize: '9px', color: '#aaa' }}>
                      {player.edad} · {player.posicion} · {player.sexo === 'M' ? 'Masculino' : 'Femenino'} · Sem {player.semestre}
                    </p>
                  </div>
